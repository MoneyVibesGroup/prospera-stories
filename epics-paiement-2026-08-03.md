...
preuve — puis le registre administrable, le barème contractuel, et un second adaptateur
d'une autre nature, qui est la seule preuve que le port tienne sa promesse. *Couvre FR-P07→P12, FR-P58, NFR-5, AR-08.* **43 pts** ⚠️ *estimés ~13 le 2026-08-03, avant STORY-603, STORY-289 et STORY-600 ; 38 hors STORY-247, écartée pour R4.*
...
### STORY-600 — L'adaptateur du schéma interopérable, sur le bac à sable de l'API-Business

En tant qu'**organisation cliente de l'UEMOA**, je veux qu'une demande de paiement puisse partir vers le schéma de paiement instantané interopérable de la BCEAO et atterrir chez le participant agréé qui tient mon compte, afin que la promesse « l'ajout du SPI est une configuration, pas une réécriture » soit tenue par un adaptateur éprouvé de bout en bout **avant** que la convention de production existe. *(FR-P07, FR-P08, FR-P09, FR-P10, FR-P11, FR-P12 · AD-5, AD-6, AD-14, NFR-5)*

> 🏁 **Elle ferme EPIC-036 pour de bon** (hors STORY-247, écartée pour le risque carte). Le port de STORY-245 promet des fournisseurs interchangeables ; jusqu'ici il ne l'a prouvé que sur **un** fournisseur, d'**un seul** genre, avec un checkout hébergé et une clef marchande. Un second adaptateur d'une **autre nature** — pas de checkout, pas de clef marchande, une destination en trois morceaux — est la seule preuve qui vaille.
>
> ⚡⚡ **La clef de plateforme n'était pas interdite pour ce qu'elle EST, mais pour ce qu'elle DÉSIGNAIT.** STORY-246 a refusé une clef unique parce que, chez FedaPay, la clef **est** la destination : appeler avec la clef de Prospera, c'est encaisser sur le compte de Prospera, et le changement de régime juridique que NFR-1 interdit (R1) aurait eu l'apparence d'une ligne de configuration. Le schéma sépare les deux : l'accès dit **qui appelle**, le triplet de STORY-599 dit **où l'argent atterrit**, et il voyage dans la requête. Le refus ne se transpose donc pas — mais la **garantie**, elle, doit se transposer, et elle est déjà écrite : un alias ne s'ouvre qu'avec le code marchand et le participant qui étaient là quand il a été scellé. Un accès partagé ne peut pas envoyer la demande d'une organisation vers la destination d'une autre.
>
> ⚡⚡ **Le SPI n'a pas de checkout : le payeur ne va pas chez le fournisseur, c'est la demande qui va au payeur.** `PresentationCheckout` a deux membres, `REDIRECTION` et `API_DIRECTE`, et un paiement poussé n'est ni l'un ni l'autre. Le loger dans `REDIRECTION` ferait voyager une charge utile de QR normalisé dans un champ nommé `urlDeCheckout` — la page publique en ferait un lien, et un lien vers `00020101…` ne mène nulle part. Le loger dans `API_DIRECTE` le ferait dépendre de la machinerie de STORY-247, écartée pour R4, alors que **rien ici ne touche une donnée de paiement**. C'est donc une **troisième forme de présentation**, et c'est la leçon de STORY-599 reprise un cran plus haut : ce qui n'entre dans aucun membre d'une union fermée est un changement de schéma, jamais une configuration.
>
> ⛔ **Et un paiement poussé n'a pas de retour.** Personne ne revient d'un checkout qu'il n'a pas ouvert : `urlDeRetour` ne veut rien dire ici, et **la seule chose qui puisse faire progresser une demande est la notification signée** (AD-4). Chez FedaPay, une redirection donnait au moins un indice au payeur et à la page ; ici, sans la boîte de réception de STORY-256, un encaissement réussi reste invisible **pour toujours**, et il l'est en silence.

> ⛔⛔ **CE QUE LA DOCUMENTATION DU SCHÉMA A CORRIGÉ DANS CETTE FICHE, LE 2026-09-06.** Elle a été écrite sans accès à la documentation, qui n'est lisible qu'avec un compte. Quatre de ses affirmations étaient fausses, et deux d'entre elles touchaient STORY-599, déjà livrée. **Le fait qu'elles se corrigent sans rien démonter est le vrai résultat** : ce qui tenait, c'était la FORME du problème — un triplet, un lien qui l'authentifie, un seul scellé — et non le contenu des champs.
>
> - **L'alias n'est pas un numéro de téléphone.** L'adresse de paiement du schéma est un identifiant de 36 caractères **généré par la plateforme**, sans donnée personnelle, et c'est la **seule** qu'un QR interopérable accepte. Un numéro de téléphone est un **autre type d'alias**, réservé aux personnes physiques et refusé en QR — donc inutilisable pour le compte d'encaissement d'une organisation. Le code de STORY-599 n'aurait accepté **aucune destination réelle**. Corollaire : **on ne normalise pas ce qu'on n'a pas produit**.
> - **Le « code marchand » n'est pas un champ de la destination** : c'est encore un autre type d'alias, réservé aux personnes morales et aux parcours USSD. Ce qui manquait à côté de l'adresse est le **numéro de compte chez le participant** — celui que l'interface adresse.
> - **La clef de plateforme n'a jamais été en question.** Les identifiants sont créés à l'enrôlement du client business **auprès de son participant** : ce sont ceux de l'organisation, et la règle de STORY-246 s'applique telle quelle. ⚡ En revanche **l'interface est exposée PAR LE PARTICIPANT** — l'origine est une propriété du **raccordement**, pas du fournisseur, et un raccordement ne sert **qu'un** établissement.
> - **La vérification ne peut pas comparer le participant** : l'annuaire ne rend qu'un **nom** et un **pays**. On compare donc le pays, seul champ rendu qui ait une forme canonique — et jamais le nom, dont la comparaison se relâcherait jusqu'à ne plus rien refuser.
>
> ⛔⛔ **ET UN BLOCAGE PRODUIT, TRANCHÉ PAR LE PO LE 2026-09-06 : ON NE DEMANDE PAS D'ARGENT À UN INCONNU.** Le schéma exige l'**adresse du payeur** pour pousser une demande, et il **interdit le QR** — le chemin du payeur anonyme — pour le paiement à distance. Une créance dont le payeur n'a pas d'adresse connue **est hors de ce canal**, et non un cas dégradé : l'organisation **enrôle** l'adresse de son client, et l'initiation refuse sinon, **avant d'ouvrir le coffre et avant tout appel**, devant celui qui peut corriger.
>
> ⚡ **La présentation poussée ne porte finalement AUCUNE donnée.** Le premier jet lui donnait une charge utile de QR ; le QR existe, mais il est interdit à distance, et la création d'une demande **ne rend aucun corps de réponse**. Il n'y a donc ni adresse où envoyer le payeur, ni champ à lui demander, ni image à lui montrer — et **notre référence est la référence** chez le fournisseur. Lui inventer un champ à afficher aurait été lui donner du faux à montrer.

**Critères d'acceptation**

- **Étant donné** l'accès au bac à sable de l'API-Business **quand** l'adaptateur s'enregistre **alors** il porte un identifiant qui nomme la **porte** et non le schéma — un second accès au même schéma, par l'API propre d'un participant, ne doit pas obliger à renommer celui-ci (`fournisseur` dit *par qui l'appel part*, `participant` dit *chez qui l'argent atterrit*) — **et** il déclare la **seule** nature de compte du schéma : pour les trois autres il n'a ni clef à présenter ni destination à composer, et un compte marchand déclaré chez lui est refusé à la saisie, sous le code qui lui est propre (STORY-599).
- **Étant donné** les couvertures qu'il déclare **alors** elles sont le **périmètre du schéma** — les huit États de l'union, une seule devise, une seule méthode — et **ce n'est pas à elles de dire où Money Vibes a réellement ouvert** : le registre plateforme **ne peut que retrancher** (STORY-289), et c'est lui seul qui ouvre un marché, sur une décision écrite. *(Corrigé le 2026-09-06 : la fiche demandait une couverture « constatée sur le bac à sable », ce qui aurait fait refuser, pour une raison d'environnement de test, un pays que le schéma sert.)* **Et** aucune ne porte de **grille publiée** : le coût d'un encaissement sur le schéma est une clause de la convention du participant, donc une organisation sans barème contractuel est refusée par `TARIF_NON_CONTRACTE` (STORY-603) — à l'émission, devant celui qui peut corriger, jamais devant le payeur.
- **Étant donné** un compte du schéma **quand** l'adaptateur emprunte son alias au coffre **alors** le lien est reconstruit **avec le participant et le numéro de compte chez lui** — il ne l'est pas aujourd'hui : STORY-599 a étendu le lien du côté de l'**écriture** et le seul passage par lequel un secret sort du coffre le rebâtit sans destination, si bien que le premier appel sortant du schéma échouerait sur l'authenticité. **Et étant donné** un compte dont le participant ou le numéro de compte a été réécrit directement en base **quand** une demande part **alors** elle échoue **avant tout appel sortant**, sans qu'un octet quitte le service. **Et étant donné** un compte sans destination **quand** on l'emprunte **alors** son lien est **inchangé, octet pour octet**.
- **Étant donné** l'alias emprunté **quand** on suit ce qu'il devient **alors** il part **dans le corps de la demande**, comme la destination qu'il est, et **jamais dans un en-tête d'autorisation**, jamais dans un journal, jamais dans un message d'erreur : chez FedaPay, l'identifiant du compte et la clef de l'appel étaient la même chose, et **cette coïncidence n'a jamais été une règle**. **Et** deux demandes de deux organisations, parties sous le **même** accès, portent deux destinations et ne portent rien l'une de l'autre.
- **Étant donné** un compte du schéma **quand** on le vérifie **alors** l'adaptateur **pose une question** — cet alias est-il connu, et chez quel participant — sans qu'aucun franc ne bouge (AD-14), **et refuse** si le **pays** rendu n'est pas celui du compte déclaré : c'est le seul second avis possible sur une destination que l'organisation a saisie elle-même. *(Corrigé le 2026-09-06 : l'annuaire ne rend pas le participant.)* **Et** ce qu'on consigne est l'**empreinte** de l'adresse, jamais son clair — quand la référence du compte chez le fournisseur **est** le secret, la restituer annulerait STORY-243 par la sortie de secours. **Et** il ne compare **aucun nom** : un nom de titulaire n'a pas de forme canonique, et une comparaison qu'on relâche pour cesser de refuser les bons comptes ne refuse plus rien. Les comptes que STORY-599 faisait naître `NON_VERIFIE` faute de pouvoir demander deviennent vérifiables — **en bac à sable**.
- **Étant donné** le code de l'adaptateur **quand** on le balaie **alors** il ne porte **aucune condition d'environnement** : la garde de STORY-246 s'applique à lui sans être assouplie. **Et** l'origine de l'interface est **contrôlée** : une URL absolue chiffrée, sinon le **démarrage échoue** — absente, l'adaptateur existe sans rien pouvoir initier. *(Corrigé le 2026-09-06 : la fiche demandait une liste d'origines publiées. Elle est **impossible** ici — chaque participant expose la sienne, ils se comptent par centaines, et une liste administrable **autoriserait** un hôte, ce que le critère de STORY-603 interdit à une donnée. C'est le refus de STORY-599 sur les participants agréés, rencontré une seconde fois.)* **Et le déclencheur nommé du passage en production est ailleurs, et plus fort qu'une liste** : le bac à sable **désactive** le chiffrement mutuel, la production l'**exige** avec un certificat délivré par l'autorité de la banque centrale et des jetons liés à ce certificat — aucune configuration ne le fabrique.
- **Étant donné** un service démarré sans les paramètres du bac à sable **quand** il démarre **alors** il démarre quand même (FR-P11) : l'adaptateur nomme **ce qui lui manque, jamais sa valeur**, `/health` le publie indisponible sans rien décider (STORY-249), et une demande qui lui serait routée rend `FOURNISSEUR_NON_CONFIGURE` — la demande restant ouverte (FR-P12). **Et** la garde d'inertie de STORY-599 — « aucun adaptateur ne déclare encore ce schéma » — est **supprimée**, jamais contournée : elle disait vrai jusqu'à cette story, elle serait fausse après.

> ⚠️ **Ce que la fiche ne peut pas écrire, et qui doit lui être joint avant le développement.** La documentation de l'API-Business n'est lisible qu'avec un compte : l'origine du bac à sable, la forme de l'accès, l'opération de demande de paiement, le format de la charge utile du QR normalisé et l'existence d'une résolution d'alias viennent du **compte de démonstration du PO**. Les inventer produirait des critères qui échouent au premier appel. ⚠️ **Si le bac à sable n'expose aucune résolution d'alias, l'adaptateur ne déclare pas de méthode de validation** — l'absence de la méthode **est** la déclaration (STORY-244), les comptes restent `NON_VERIFIE`, et c'est l'avant-dernier critère qui tombe, rien d'autre.

> ⛔ **Ce que la story ne fait pas.**
> - **Aucune réception de notification signée** : la boîte de réception et le corps brut sont STORY-256. ⚠️ Le contrôle de signature du schéma exige le **corps brut**, exactement comme `X-FEDAPAY-SIGNATURE` — un parseur JSON global rendrait les deux invérifiables, en silence.
> - **Aucun rendu de QR, aucune page publique** : c'est STORY-253 — qui hérite d'une obligation nouvelle, **trois formes de présentation et non deux**, dont une qui ne s'affiche pas comme un lien.
> - **Aucune mise en production**, et c'est un fait de programme, pas un reste : Money Vibes n'est pas un établissement agréé. Le déclencheur porte un nom — la convention avec un participant — et il est le même patron que les passerelles de `notification-service`.
> - **Aucune liste de participants agréés** (refus déjà tenu par STORY-599 : une liste en dur serait périmée avant d'être relue, une liste administrable **autoriserait**), **aucun remboursement**, **aucun paiement partiel**, **aucune donnée de paiement** — R4 reste entièrement hors de cette story.
> - **Aucune borne de montant contrôlée par l'adaptateur** : depuis STORY-289 elles sont contractuelles. Il déclare celles du schéma, il ne les re-contrôle pas — un contrôle qui vérifie ce que son appelant vient d'affirmer est un écho, pas un second avis.
