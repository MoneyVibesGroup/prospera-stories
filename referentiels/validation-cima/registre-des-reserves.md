...
1. **Une réserve ne disparaît jamais.** Son statut change, avec la date et la pièce qui le fonde —
   la fiche de réponse signée, ou la story qui l'a traitée. ⛔ Ce dépôt est **public** : une fiche se
   cite par un **identifiant opaque**, jamais par un nom de fichier ou une mention qui porterait le nom
   de l'expert.
...
6. **Les réserves nouvelles** qu'apporteront les fiches de réponse s'ajoutent à la suite (`R-48`…),
...
| `DÉFAUT` | défaut du produit | une story ; quand une question le cite, elle porte sur la **manière** de le corriger, jamais sur son existence |
...
| 2 | **R-02** — la liasse validée ne contient pas les comptes 80 et 87 | le compte d'exploitation générale, que l'art. 422 exige avec le bilan, n'entre pas dans le document scellé |
...
|---|---|---|---|---|---|---|---|
| **R-01** | ⛔ Le `statut` et la `miseEnGarde` du paquet ne sont **pas** servis par le jeu d'états (création, lecture, recalcul, compléments, validation, réouverture, dépôt : `stamp` forcé à `undefined`), l'export PDF/XLSX (son « Statut » est le cycle de vie), la liste des versions et leurs comparaisons, la consultation sans version, la comparaison d'exercices, la consolidation, le prévisionnel, les routes fiscales de `balance-service`, `platform-catalog-service` et `admin-panel`. Ils le sont sur onze routes : catalogue et diagnostic du référentiel, les six dry-runs d'états, référentiel et suggestions de `balance-service`, référentiel d'`assurance-service`. ⚠️ La liasse **scellée** les stocke (`liasse.statut`, `liasse.miseEnGarde`), et deux routes les ressertent **sous `liasse`**, sans les décrire au contrat — `GET …/versions/:version` et la consultation avec `?version` : la story qui traitera R-01 les lira **dans la liasse scellée**, jamais dans le paquet du jour, qui ferait porter à une pièce figée le statut d'aujourd'hui. | STORY-540 M3 ; `bilan-service` `jeu-etats-response.dto.ts:616-627` (« 🪝 Story à part », sans numéro) | spine AD-10 | `DÉFAUT` | — | `OUVERTE` | **à ouvrir — priorité 1** |
...
| **R-08** | Comptes que l'art. 432 cite et que l'art. 431 n'énumère pas : `360`, `3910`, `3930`, `3950`, `3955`, `3960`, `603`, `606`, `703`, `706`, `7909` ; coquilles de la page officielle : `6126` pour `6026`, `6905` sans point, `6029` en double (sous `602` et sous `620`, où il faut lire `6209`). Faute de ces comptes au plan, un `6030`, `6060`, `606`, `703` ou `706` est capté par `60` ou `70` : il n'alimente que `RC1` ou `RP1`, **jamais** le compte 80. | STORY-512, 515, 518, 520, 521 | art. 431, 432 | `QUESTION` | Q3 | `OUVERTE` | STORY-671 (en partie) |
...
| **R-11** | Le compte `49` doit figurer au bilan s'il n'a pu être reclassé : « il n'est pas établi de compensation entre les soldes créditeurs et les soldes débiteurs des comptes, qui doivent apparaître au bilan » ; le paquet ne le route vers aucun poste — un `49` non soldé bloque la validation de la liasse (`COMPTES_NON_AFFECTES`). | STORY-540 M6-e | art. 432 (compte 49), 433 | `ÉCART AU TEXTE` | Q1 | `OUVERTE` | à ouvrir |
...
| **R-13** | Le modèle porte le résultat en **`87`** (« excédent avant affectation » au passif, « pertes de l'exercice » à l'actif). Le paquet ajoute le résultat calculé à `CP1` **pour le seul total**, la ligne publiée ne le contient pas, et range `88` dans `CP1`. Quel compte porte le résultat au bilan CIMA, et la ligne publiée doit-elle le contenir ? | STORY-540 M6-e ; `bilan-production.service.ts` (total du passif) | art. 432 (comptes 87, 88), 433 | `QUESTION` | Q1 | `OUVERTE` | à ouvrir |
...
| **R-20** | `RT` n'est pas le solde du compte 80 : il laisse dehors frais de personnel, impôts, travaux, frais divers, subventions et produits accessoires, que l'art. 432 range dans le compte 80, et les charges des placements (`67`) alors qu'il en retient les produits (`77`) ; il reste servi, avec son libellé d'amorce, dans un état `COMPTE_RESULTAT` qui n'est **pas** un modèle du Code. | STORY-521 ; `README-cima-assurances.md` | art. 432 | `QUESTION` | Q1 | `OUVERTE` | — |
...
| **R-39** | L'état C10b du module est tenu **par catégorie Vie / Non-Vie** ; l'art. 433 l'établit « pour l'ensemble des opérations d'assurances dommages réalisées dans le pays et pour chacune des catégories d'assurances dommages définies à l'article 411 », et le modèle annexé au règlement n° 006/2024 est « à répéter pour toutes catégories des assurances terrestres », ses tableaux A et B — les seuls republiés — sur dix exercices. Tableau F non produit, tableau A renvoyé à STORY-514. | STORY-516 ; STORY-540 M9 ; règlement 006/2024 | art. 422, 433 | `QUESTION` | Q6 | `OUVERTE` | — |
...
## I bis. Ce que la validation ne peut pas couvrir

| ID | Réserve | Source | Texte | Nature | Question | Statut | Story |
|---|---|---|---|---|---|---|---|
| **R-47** | Une **surcharge de rattachement** validée par une organisation l'emporte sur la table packagée — comptes 80 et 87 compris — **sans changer l'empreinte** du paquet : la validation d'une version ne couvre pas la liasse d'une organisation qui a surchargé ses rattachements. La bascule à `certifie` devra dire ce que devient une liasse surchargée. | revue de code de STORY-540 ; `bilan-service` `table-de-passage.service.ts` (surcharges prioritaires), `comptes-cima-production.service.ts` | — | `DÉCISION PO` | — | `OUVERTE` | à trancher avec la bascule |
