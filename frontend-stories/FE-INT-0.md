...
**API :** transverse — auth-service (:3001), expert-comptable (:3000), kyc-service (:3002), préfixe `/api/v1`
...
je veux **un client API qui route chaque appel vers le bon service par son préfixe et inclut le préfixe global `/api/v1`, avec des types générés depuis les OpenAPI réels**,
...
2. **Préfixe global `/api/v1` omis** partout (les 3 services montent `setGlobalPrefix('api') + URI versioning v1`).
...
**Inclus :**
- **Refactor `src/lib/api/api-client.ts`** : résolution de la base par **préfixe logique** → `NEXT_PUBLIC_AUTH_URL` (:3001) pour `/auth` & `/users`, `NEXT_PUBLIC_EC_URL` (:3000) pour `/ec` (→ `/tenant*`), `NEXT_PUBLIC_KYC_URL` (:3002) pour `/kyc`. Concaténer le **préfixe global `/api/v1`**. Bearer + refresh silencieux inchangés (cross-service : 1 seul token RS256).
- **BFF (`src/lib/auth/bff.ts`, `idpUrl`)** : cible auth-service (`NEXT_PUBLIC_AUTH_URL`) + `/api/v1`.
- **Upload KYC (`src/features/kyc/api/upload-document.ts`)** : cible `NEXT_PUBLIC_KYC_URL` + `/api/v1/kyc/documents` (XHR progress conservé).
...
1. Un appel `/auth/...` part vers `NEXT_PUBLIC_AUTH_URL` + `/api/v1`, `/kyc/...` vers `NEXT_PUBLIC_KYC_URL` + `/api/v1`, `/ec/...` vers `NEXT_PUBLIC_EC_URL` + `/api/v1` — vérifié par tests unitaires du client.
...
5. Contre le stack STORY-075 (`docker compose up`) : un `GET /api/v1/tenant/state` (EC) et un `GET /api/v1/kyc/status` (kyc) partent bien sur des bases distinctes avec le même token.
...
## ⚠️ Correction de contrat (2026-07-17)

La rédaction initiale de cette story disait préfixe global **`/api`**. C'est **faux**.

**Contrat réel, figé et vérifié docker par la STORY-075 backend (done le 2026-07-14) :**

| Élément | Valeur réelle |
|---|---|
| Préfixe global | **`/api/v1`** (`setGlobalPrefix('api')` **+ URI versioning v1**) |
| JWKS | `/.well-known/jwks.json` — **non préfixé** |
| Swagger UI / JSON | `/api/docs` · `/api/docs-json` |

Le `v1` vient du **versioning d'URI de Nest**, qui s'ajoute au `setGlobalPrefix('api')` : lire le seul
`setGlobalPrefix` menait à conclure `/api`, et c'est exactement l'erreur commise ici. Partir sur `/api`
ferait échouer **tous** les appels des 4 services (404), et le retrofit taperait à côté de sa cible.

**Bases par service (gateway = option B, direct-par-service) :**
`NEXT_PUBLIC_AUTH_URL` :3001 (`/auth`, `/users`, `/organizations`, `/admin/*` IdP) ·
`NEXT_PUBLIC_EC_URL` :3000 (`/tenant/state`, `/auth/me`, `/billing/plans`) ·
`NEXT_PUBLIC_KYC_URL` :3002 (`/kyc/*`) · `NEXT_PUBLIC_CATALOG_URL` :3003 (`/catalog/*`) ·
`NEXT_PUBLIC_ADMIN_URL` :3010 (BFF admin — consommé par AP-01, pas par l'app cliente).

**Statut de la dépendance : LEVÉE.** STORY-075 est done — cette story n'attend plus rien.
