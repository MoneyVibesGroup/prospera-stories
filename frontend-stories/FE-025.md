# Story FE-025 : Import de balance Sage 100 — upload, aperçu dry-run, persistance
...
- **Une story = une branche.** Branche : `fe-025`. Commits préfixés `FE-025`.
...
- La saisie manuelle (FE-026), la consultation (FE-027).
...
qui est retiré du périmètre**, et il appartient à **FE-048** (STORY-088, profil d'import réutilisable, S17).
...
- **Montants en unités mineures XOF** : `totalDebiteur`, `totalCrediteur`, `ecartEquilibre` et les
  ...
  flottante** sur ces montants. Voir FE-026, qui porte la règle complète.
...
- [ ] Commits sur `fe-025`, préfixés `FE-025`.
...
⚠️ **Propre à cette story** : l'aperçu `dryRun='true'` **n'écrit rien, mais reste
un `POST`** — et le `DossierScopeGuard` arbitre sur la **méthode HTTP**, pas sur
l'intention. Sur un dossier archivé, l'aperçu lui-même repart donc en
`409 DOSSIER_ARCHIVE`. C'est cohérent (analyser un fichier qu'on ne pourra pas
persister n'avance à rien), mais ça ne se devine pas à la lecture du contrat.
