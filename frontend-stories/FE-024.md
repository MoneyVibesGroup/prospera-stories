# Story FE-024 : Module Atelier — shell, navigation, gate d'accès et choix de source
...
- **Une story = une branche.** Branche : `fe-024`. Commits préfixés `FE-024`.
...
|---|---|---|---|
| `sage` | Export Sage 100 (Excel/CSV) | STORY-086 | **livré** → FE-025 |
| `direct` | Saisie / ingestion directe | STORY-101 | **livré** → FE-026 |
...
- Tout import ou saisie (FE-025 / FE-026), toute consultation (FE-027).
...
- [x] 7 critères d'acceptation validés ; tests verts.
- [x] Aucune décision d'autorisation prise côté client.
- [x] `lint` / `typecheck` / `test` / `build` verts (local + CI).
- [x] Décision de préfixe logique tranchée **et écrite** dans `services.ts`.
- [x] Statut mis à jour dans les trackers.
- [x] Commits sur `fe-024`, préfixés `FE-024`.

---

## Integration Gate — exécuté (2026-07-24, stack docker `origin/dev` `3bf4b5f`)

Gate confronté au **vrai** `balance-service` :3007, org créée via l'IdP réel.

| Situation de l'org | Réponse observée | `code` |
|---|---|---|
| E-mail non vérifié | **403** | ⚠️ **aucun** (voir écart 1) |
| E-mail vérifié, KYC non approuvé | **403** | `KYC_NOT_APPROVED` |
| KYC approuvé, aucun entitlement | **403** | `BALANCE_NOT_ENTITLED` |
| Entitlement `balance` ACTIVE | **409** (gate franchi) | `REFERENTIEL_UNRESOLVED` |

Conforme à l'exigence : une org sans entitlement reçoit bien **403 `BALANCE_NOT_ENTITLED`** — ni 401, ni 500.

### Écarts relevés (à traiter hors de cette story)

1. **403 `EMAIL_NOT_VERIFIED` sans `code`.** Le refus e-mail vient du guard **global**
   `EmailVerifiedGuard`, qui répond `{statusCode, error, message}` **sans champ `code`**, alors que
   `BalanceAccessGuard` en pose un. Le front ne peut donc pas nommer ce motif à partir du 403 seul ; il
   le couvre par le reflet local (`emailVerified` du jeton) et par la garde d'app globale. **Ticket
   backend à ouvrir** pour aligner le guard e-mail sur le contrat `{ message, code }`.

2. **Le registre de modules front déclare `atelier`, le backend ouvre sur `balance`.**
   `balance-service` ne projette que les `entitlement.changed` dont `moduleCode == "balance"`
   (`BALANCE_MODULE_CODE`). Le registre FE-014 (`features/modules/config/registry.ts`) déclare le module
   avec `code: "atelier"`. **La carte du module en accueil ne s'ouvrira donc jamais**, quel que soit
   l'octroi. FE-024 joint sur `balance` (l'autorité) ; **le registre FE-014 reste à corriger** — hors
   périmètre de cette story car il touche l'écran d'accueil.

3. **Objets imbriqués non typés au contrat.** `referentiel`, `paquetFiscal` et `stamp` sont déclarés
   côté backend avec `@ApiProperty({ example })` **sans schéma** → l'OpenAPI les génère en
   `Record<string, never>`. Les écrans s'appuient sur les champs réellement typés (`libelle`,
   `checksum`, `planCount`, `fiscal.statut`…) plutôt que de caster un contrat non garanti.
...
⚠️ **Propre à cette story** : le **gate du module** n'a pas changé de portée. Il
reste org-keyed (il sonde `referentiels/actifs`), et c'est délibéré : le droit
d'accès à l'Atelier appartient à l'organisation. Le scoper ferait re-tester
l'accès à chaque bascule de dossier, donc clignoter le gate sur un droit stable.
