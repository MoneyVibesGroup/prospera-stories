# Story FE-026 : Saisie manuelle de balance + contrôle d'équilibre live

Status: in-progress (impl. livrée sur `fe-026`, maquette validée 2026-07-25)

> **⚠️ UX RÉVISÉE APRÈS VALIDATION PO (2026-07-25).** Le PO a écarté la grille
> comptable brute (compte/débit/crédit) — « pas compréhensible pour qui vient de
> Sage/Excel ». Modèle retenu (options 1 + 3) : **saisie par sections familières**
> (Achats & charges, Ventes & produits, Trésorerie, Clients & fournisseurs) où
> chaque ligne = **libellé + montant + sens** (Débit/Crédit, un seul montant, pas
> deux colonnes), et le **compte SYSCOHADA est deviné pour l'utilisateur**
> (éditable). Le mapping est **piloté par le référentiel** — SYSCOHADA (SN/SMT),
> **microfinance (SFD-BCEAO)**, et **assurance (CIMA)** via le même mécanisme dès
> que le référentiel entrera dans l'enum backend. Le § « Périmètre » ci-dessous
> garde la logique de contrat (montants, checksum, équilibre) qui, elle, ne change
> pas ; seule la **surface de saisie** est celle décrite ici.
>
> **Trou de contrat tracé (mapping) :** la table `libellé → compte` faisant
> autorité, **par référentiel**, n'existe pas côté `balance-service`. Elle est,
> pour l'instant, un **dictionnaire client intérimaire** (`config/plan-comptes.ts`,
> annoté) — aide à la saisie, le serveur restant seul juge. → **story backend
> à ouvrir** : endpoint/service de mapping référentiel-aware (voir
> `stories/STORY-1xx-mapping-libelle-compte.md`).
...
**Dépendances :** FE-024 (shell + routage `:3007`)
...
- **Une story = une branche.** Branche : `fe-026`. Commits préfixés `FE-026`.
...
- La **validation** d'une balance (passage `BROUILLON` → `VALIDÉE`) : **il n'existe aucune route pour le
  faire** — voir *Trou de contrat* ci-dessous. C'est l'objet de FE-027 une fois la route ouverte.
- Le `statutPreuve` **calculé** : le serveur le dérive ; le front peut l'**anticiper** pour informer, mais
  ne l'envoie jamais (il le déduit des `niveauPreuve` — voir FE-027).

---

## ✅ Trou de contrat — RÉSOLU le 2026-08-08 (FE-058)

> **Réserve d'origine, levée :** *« Le service crée toujours une balance en `etat: 'BROUILLON'` et n'expose
> aucune route pour la faire passer `VALIDÉE`/`REJETÉE`. La logique existe (`BalanceService.marquerEtat`,
> immuabilité incluse) mais n'est câblée à aucun contrôleur. »*

**STORY-145** a livré `POST /balances/:id/valider` et `POST /balances/:id/rejeter` le 2026-08-03, et
**FE-058** les a câblés à l'écran de détail le 2026-08-08. `GAP-balance-validation-etat` est **clos**.

Ce que cette story livre reste inchangé : **la production d'un brouillon**. L'action de validation vit sur
`/atelier/balances/[id]` (FE-027 amendée par FE-058), pas sur l'écran de saisie — et il n'a jamais été
inventé de `PATCH .../valider` côté front, ce qui a permis de câbler la vraie route sans rien défaire.
...
6. Sans entitlement, **403** traité par son `code` (règle FE-024).
...
- [x] Aucune UI de validation d'état livrée **ici** ; ticket backend `marquerEtat` ouvert et référencé
      → **soldé** par STORY-145 (2026-08-03) puis **FE-058** (2026-08-08), sur l'écran de détail.
- [ ] `lint` / `typecheck` / `test` / `build` verts (local + CI).
- [ ] Statut mis à jour dans les trackers.
- [ ] Commits sur `fe-026`, préfixés `FE-026`.
...
⚠️ **Propre à cette story, et c'est le piège le plus coûteux du re-scopage** : le
`dossierId` **n'entre PAS dans le checksum**. Le sceau ne couvre que le contenu
métier (exercice, source, référentiel, version, lignes) ; `orgId`, `auteur` et
désormais `dossierId` sont **dérivés côté serveur**. L'ajouter « par cohérence »
ferait diverger le hash du recalcul serveur et refuserait en `400` **toute**
saisie directe.

⚠️ Et l'**idempotence est désormais bornée au dossier** : l'index d'unicité est
passé d'un préfixe `orgId` à `dossierId`. Deux dossiers peuvent porter la même
version du même exercice sans se percuter — le cas normal d'un cabinet.
