...
- [ ] AC-1 — Une seule commande amorce les **quatre** étapes et rend un rapport lisible, étape par
      étape, avec ce qui a été fait et ce qui existait déjà.
- [ ] AC-2 — ⛔ **Aucune écriture directe en base de données.** Le script n'ouvre aucune connexion
      Mongo ; il n'appelle que des routes HTTP existantes. Garde de balayage sur ses dépendances.
- [ ] AC-3 — ⛔ Le script **refuse de s'exécuter** si l'environnement n'est pas nommé parmi ceux
      autorisés. Le refus est un code de sortie non nul, pas un message.
- [ ] AC-4 — **Idempotent** : deux exécutions successives rendent le même état final et le second
      rapport dit « déjà en place » sur chaque étape.
- [ ] AC-5 — Les pièces de conformité déposées sont **marquées comme factices** dans leur nom et
      leur contenu.
- [ ] AC-6 — Après exécution, un test de bout en bout montre Money Vibes **émettant une demande** sur
...
- [ ] AC-7 — Une commande symétrique **retire** ce que le seed a posé, ou le document dit
      explicitement que la remise à zéro se fait par la base de développement.
- [ ] AC-8 — Le document du dépôt dit, en une phrase, **ce qu'il faudra faire à la main** en
